...
> Ngày lập: 2026-06-10. Cập nhật gần nhất: 2026-07-04 (thêm **Wave 3** — mục 3, F23+).
> Tiến độ: ✅ **Phase 0–6 hoàn thành** (hơn 23 tính năng + phiên rà soát chất lượng ~30 bug fix + 71 test) + **v0.1.3 → v0.1.9 đã phát hành** (gộp tab/mở nhóm/ảnh nền · workspaces/notes/tuỳ biến terminal/accent/tmux · copy-dán chuột · plugin system v1/theme studio/favorites · ảnh nền URL · fix login-script Bulk/Monitor · plugin Access Log Analyzer + dock panel). Chi tiết tính năng: [docs/USER-GUIDE.md](docs/USER-GUIDE.md). Trạng thái việc còn lại: [ROADMAP.md](ROADMAP.md).
...
| F22 | **SFTP qua nested-ssh jump** ✅ | SFTP vào máy nội bộ chỉ vào được khi đứng trên gate (web-01 chỉ `ssh` được từ gate bằng key của gate): bọc lớp SFTP của ssh2 quanh exec `ssh <đích> -s sftp` chạy trên gate. Tự suy ra đích từ login script "ssh …". Termius/Tabby không làm được — chúng chỉ SFTP vào đúng máy kết nối TCP |

### Wave 3 — đề xuất mở rộng (thêm 2026-07-04, chưa cam kết lịch)

> Trạng thái + thứ tự ưu tiên theo dõi ở [ROADMAP.md](ROADMAP.md) mục "Wave 3". Các đề xuất trùng với parity/wave cũ **không** cấp ID mới — chúng nằm ở ID gốc: P45 (SFTP remote↔remote), P46 (transfer queue), P17 (Windows Hello), P51 (log bookmarks), P36/P39/P40/P31, F03 (runbook/diff), F04 (alert ngưỡng), F07 (WoL/discovery), F15 (tunnel auto-start).

| # | Tính năng | Mô tả |
|---|-----------|-------|
| F23 | **Shell integration (OSC 133)** | Inject prompt marker → biết ranh giới từng lệnh: exit code + thời gian chạy cạnh mỗi lệnh, nhảy giữa các prompt, click lệnh cũ để chạy lại. Nền cho F24/F26 + AI đọc context |
| F24 | **Command history per host** | Lịch sử lệnh theo host, mã hoá trong vault, search Ctrl+R tầng app; sống sót cả khi server xoá `.bash_history`; nền cho autocomplete P36 |
| F25 | **Trigger & highlight regex** | Tô màu dòng output khớp pattern; rule "thấy X → notification/âm thanh" |
| F26 | **Báo khi lệnh dài xong** | Tab không focus mà lệnh xong → desktop notification |
| F27 | **Protective mode** | Host production: confirm trước khi gửi lệnh nguy hiểm (`rm -rf`, `DROP DATABASE`…); quan trọng khi Broadcast |
| F28 | **Diff/sync 2 thư mục local↔remote** | So sánh mtime/size/checksum, đồng bộ chọn lọc kiểu rsync |
| F29 | **Watch & auto-upload** | Theo dõi thư mục local, file đổi tự đẩy lên remote |
| F30 | **Tail viewer** | Panel tail -f file log từ SFTP: follow/filter/highlight, không chiếm terminal |
| F31 | **Nén/giải nén remote** | tar.gz/unzip qua exec (tái dùng hạ tầng login-script) |
| F32 | **Lịch sử metrics + đồ thị** | Lưu sample monitoring vào SQLite, đồ thị 1h/24h; đi cặp alert F04 + webhook Slack/Telegram/Discord |
| F33 | **Process viewer** | Panel top-like: sort CPU/RAM, kill có confirm |
| F34 | **Systemd manager** | List/start/stop/restart service, xem `journalctl -u` |
| F35 | **Cron manager** | Đọc/sửa crontab qua UI, diễn giải lịch human-readable |
| F36 | **Disk usage explorer** | Drill-down dung lượng thư mục (`du`), UI kiểu ncdu |
| F37 | **Package updates checker** | Quét update apt/yum trên cả fleet qua Bulk, bảng tổng hợp |
| F38 | **Security audit nhanh** | 1 nút: failed logins, port mở, fail2ban, cert TLS sắp hết hạn → báo cáo (hợp làm plugin mẫu) |
| F39 | **Uptime/port watcher nền** | Ping/check port định kỳ cả fleet, chấm xanh/đỏ ở sidebar |
| F40 | **Scheduled jobs** | Chạy snippet/bulk theo lịch, lưu kết quả từng lần, báo khi fail |
| F41 | **TOTP trong vault** | Seed 2FA mã hoá → sinh mã 6 số; auto-điền khi login script gặp prompt OTP (ghép engine F21) |
| F42 | **SSH key rotation wizard** | Sinh key mới → đẩy authorized_keys loạt host → xác minh → gỡ key cũ |
...
| F45 | **Clipboard tự xoá** | Copy secret từ vault → tự xoá clipboard sau ~30s |
| F46 | **AI giải thích output đang chọn** | Bôi đen output → chuột phải "Giải thích" → trả lời trong panel dock (tái dùng dock v0.1.9) |
| F47 | **AI sinh lệnh inline** | Gõ `#` + mô tả → AI đề xuất lệnh, Enter chấp nhận (kiểu Warp AI) |
| F48 | **AI chẩn đoán sự cố** | Chuỗi lệnh chẩn đoán từng bước có approval, đọc output đề xuất bước tiếp (agent mode F09) |
| F49 | **AI tóm tắt phiên** | Từ session/recording → note bàn giao "đã làm gì trên máy nào" (ghép F18) |
| F50 | **MCP server** | Expose app làm MCP server cho Claude Code/Desktop điều khiển — chưa client nào có |
| F51 | **Plugin panel HTML sandbox** | Panel iframe sandbox cho plugin vẽ đồ thị/bảng (thuộc gói Plugin v2) |
| F52 | **Plugin index kiểu git** | JSON trên GitHub liệt kê plugin cộng đồng, cài từ URL — marketplace 0 server |
| F53 | **Tray + chạy nền** | Đóng cửa sổ nhưng tunnel/monitoring sống trong tray (đi cặp F15) |
| F54 | **Deep link** | `infra-companion://host/<id>` — click link trong wiki/ticket mở đúng session |
