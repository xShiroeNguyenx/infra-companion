...
> | **v0.2.13** | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.13`, CHƯA commit/tag, CHƯA TEST GUI.** = **F05 Cloud import, mảnh DigitalOcean** (mở từ tab "Tất cả tính năng" / lưới Dashboard / palette — theo lệ v0.2.12, công cụ mới KHÔNG chen vào menu `⋯`). Dán API token (chỉ cần scope đọc; app chỉ gọi đúng MỘT endpoint GET /v2/droplets, không có đường nào ghi lên DigitalOcean) → lấy danh sách droplet → tick chọn → thành host: IP public trước, rơi về private cho máy chỉ trong VPC (có nhãn); gốc gác (droplet id · region · image · tags) ghi vào **notes** của host. **Dedupe theo địa chỉ**: droplet đã có host trùng địa chỉ bị KHOÁ CHỌN ("đã có"), guard áp cả trong cùng một lượt → import lại không nhân đôi danh sách; group đích mặc định "DigitalOcean" **tái dùng giữa các lần** (khác chủ đích với group có dấu ngày của ssh_config — bên đó là snapshot, bên này là nguồn đọc lại). User/key để trống = **kế thừa group** (cách thực dụng để đặt auth một lần cho cả lô). **Token theo đúng pattern `ai_api_key`**: mã hoá DEK trong `meta` (KHÔNG cần migration — vault vẫn v18), không bao giờ qua IPC sang renderer (chỉ có `hasToken`), và **chỉ lưu SAU khi fetch thành công** — token gõ nhầm không đè token đang dùng được. Lỗi liệt kê trả về **dạng MÃ** (`noToken`/`unauthorized`/`http`/`timeout`/`network`/`badResponse` + detail), renderer dịch lúc render (bài học lưu-lỗi-thô v0.2.12). Kiến trúc: parse trang + ghi vault là **hàm thuần** `packages/core/src/importers/digitalOcean.ts` — `importDroplets` nhận interface hẹp `DropletImportVault` nên **test bằng vault giả trong RAM, 0 lần argon2** (12 test); main chỉ lo mạng (`net.fetch` + AbortController 15s theo khuôn marketplace, phân trang 200/trang trần 25 trang, main điền cờ `exists` vì chỉ main nhìn được vault). IPC `IMPORT_DO_CONFIG`/`SET_TOKEN`/`LIST`/`RUN`; modal `DoImportModal.tsx` (effect nạp config để deps RỖNG — không đưa hàm vào deps); icon 💧 (droplet — đúng nghĩa đen). **+ sửa cặp icon trùng CÓ SẴN**: `menu.keyRotate` và `menu.sync` cùng 🔄 từ v0.2.11 (thêm keyRotate mà không chạy lại kiểm icon) → keyRotate đổi sang ♻️, script kiểm giờ đọc `toolCatalog.ts` tự động: **28 công cụ / 28 icon / 0 trùng**. Docs: CHANGELOG `[0.2.13]` + README (v0.2.13, bullet import DO, test-count 1356, Known Limitations, Roadmap) + USER-GUIDE **§15I mới** + ROADMAP F05 (mảnh DO ✅, còn lại AWS/GCP/Azure/Hetzner + auto-group theo tag + refresh định kỳ) + landing hero `v0.2.13`. **Cổng chất lượng: typecheck 3 package · 1356 test xanh (70 file, chạy TỪ `packages/core` bằng Electron) · build sạch · grep bundle có ghi chú `[0.2.13]` và version `0.2.13` · soát IP public 0 dòng.** **CHƯA test GUI** — muốn thử thật cần một tài khoản DigitalOcean có droplet; các nhánh lỗi (401, timeout, sai URL) và luồng token-lưu-sau-fetch phải thử tay. |
> | **v0.2.12** | ✅ **ĐÃ PHÁT HÀNH** — commit `210ad1b` + tag `v0.2.12` trỏ đúng HEAD (xác nhận 2026-09-04 bằng `git rev-list` + `git ls-tree` có `FeaturesTabView.tsx`/`toolCatalog.ts`). Mô tả lúc chuẩn bị release giữ bên dưới: |
> | ~~v0.2.12 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.12`, CHƯA commit/tag, CHƯA TEST GUI.** = **sửa lỗi từ test GUI của user + gọn menu**. ⚠️⚠️ **v0.2.11 ĐÃ PHÁT HÀNH KÈM MỘT LỖI NẶNG**: `useT()` trả về **hàm MỚI mỗi lần render**, mà `CronModal`/`DiskUsageModal` lại để `t` trong mảng dependency của effect → *effect → gọi SSH → setState → render → `t` đổi identity → effect lại*, **vòng lặp vô hạn đọc `crontab -l` / `du`+`df`**. Trên màn hình nhìn như "giật liên tục", trên server là dòng lệnh chạy không ngừng. Sửa: bỏ `t` khỏi deps + **lưu lỗi dạng THÔ, chỉ dịch lúc render** (đúng hơn: đổi ngôn ngữ thì thông báo đổi theo). Đã quét cả repo — còn 2 chỗ có `t` trong deps (`CommandPalette:90` là `useMemo`, `SettingsModal:195` chỉ gắn/gỡ listener) nhưng **vô hại** vì không IO và không setState → để nguyên. **Bài học chung: KHÔNG BAO GIỜ đưa `t` (hoặc bất kỳ hàm từ hook) vào dependency của effect có IO.** Các sửa khác: (1) ba hộp thoại Cron/Disk/Tail **không tự chọn host đầu tiên** nữa — Disk còn tệ hơn vì nó *tự quét ngay khi mở*, tức mở nhầm là đã nối SSH vào một máy không chọn; giờ bắt đầu bằng "— Chọn máy —". (2) **`Modal` trong `ui.tsx` chưa hề vẽ nút ×** → mọi popup đều thiếu, và những cái đặt `closeOnBackdrop={false}` thì `Esc` là đường ra duy nhất; thêm × vào khung dùng chung là tất cả có ngay. (3) **Menu `⋯` rút còn thứ dùng hằng ngày** + tab mới **"Tất cả tính năng"** (`FeaturesTabView`, `ToolTabKind` thêm `'features'`): chia nhóm, mỗi công cụ một dòng mô tả, có ô tìm. Gộp về **MỘT danh mục dùng chung** `lib/toolCatalog.ts` cho cả menu + lưới Dashboard + tab mới — trước đó menu và lưới khai hai danh sách riêng nên đã lệch nhau. ⚠️ Cài đặt/Trợ giúp phải để `common: false` vì chúng được vẽ TAY ở cuối menu (lệ chung của app) — đặt `true` là ra hai lần, đã dính. Local dev rời menu hẳn. Lưới Dashboard xuống 3 hàng khi quá 24 mục (2 hàng thì mỗi công cụ mới làm mọi ô hẹp lại). (4) Màn xem log thêm **dropdown đường dẫn log thường gặp** (`COMMON_LOG_PATHS` ở `shared`, có test): liệt kê CẢ hai họ distro cho cùng phần mềm (`apache2` vs `httpd`, `syslog` vs `messages`) vì app không biết máy bên kia chạy gì. **+ F35 đọc SAI CHỖ, không phải sai lệnh** (user báo: máy chạy job đều mà app nói "chưa có crontab nào"): `crontab -l` chỉ đọc crontab của **user đang đăng nhập**, còn job nằm ở **root** và ở **`/etc/crontab` + `/etc/cron.d/*`** — hai chỗ nó không đụng tới. Thêm **bộ chọn phạm vi** `user`/`root`/`system`. ⚠️ `root` phải dùng **`sudo -n`**: kênh exec không có TTY nên `sudo` thường sẽ TREO chờ mật khẩu tới lúc timeout; `-n` fail ngay và `sudoDenied()` nhận ra để báo đúng lý do thay vì "chưa có crontab". ⚠️ `system` là định dạng **6 TRƯỜNG** (có cột USER giữa lịch và lệnh) — không tách thì lệnh hiện ra kèm cả tên user, sai kiểu khó thấy vì phần lịch vẫn đúng; `system` cũng **chỉ đọc** (nhiều file). Đọc nhiều file bằng **`tail -n +1`** (tự in dấu `==> path <==`) thay vì vòng `for` — ít cú pháp shell thì ít chỗ vỡ qua hop login-script. ⚠️ **Test bắt lỗi thật**: `sudoDenied` ban đầu ràng buộc thứ tự (`/sudo.*not allowed/`) nên bỏ sót "X is not allowed to run sudo on Y" — sudo báo cả hai kiểu, phải kiểm hai vế độc lập. **+ F37 kết quả quét không đọc được, và một con số SAI** (user báo: "quét xong nhìn kết quả không hiểu lắm"): thẻ mỗi máy in 12 tên gói đầu rồi `+423` — với máy 435 gói thì đó là bức tường tên không trả lời câu hỏi nào. Thêm `packages/shared/src/pkgSummary.ts`: `groupOf` (kernel/core/web/db/runtime/other, nhận cả tên Debian lẫn RHEL), `needsReboot`, `summarizeUpdates`, `summarizeFleet`. ⚠️ Đặt ở **`shared` chứ không `core`** vì **renderer là nơi vẽ tóm tắt** mà renderer không import được `@infra/core` (§5) — cùng lệ với `commandScope`/`attention`/`logView`/`cronFormat`; test vẫn nằm ở `core` (vitest chỉ quét ở đó) và import qua `@infra/shared`. Màn hình giờ: dòng tình hình cả fleet → 2 dòng quyết định (bao nhiêu máy có **bản vá bảo mật**, bao nhiêu máy **phải reboot**) → mỗi máy MỘT CÂU + chip đếm theo nhóm → tên gói **gấp lại mặc định**, gói bảo mật tách riêng lên đầu. ⚠️⚠️ **Và sửa một lỗi đúng-loại-tệ-nhất**: máy họ RHEL báo **"435 gói, 0 bảo mật"** trong khi có cả `openssl`/`kernel`. Debian/Ubuntu ghi chữ `-security` vào **tên repo** nên code dò theo tên repo; RHEL/Rocky/Alma repo chỉ là `baseos`/`appstream`, thông tin an ninh nằm ở metadata **`updateinfo`** riêng. Thêm `securityListCommand` + `parseSecurityNames` + `applySecurityNames` (`updateinfo list --security`, vẫn `-C`, chỉ chạy cho dnf/yum và chỉ khi đã có gói; **thất bại thì bỏ qua** — thiếu nhãn còn hơn mất cả danh sách). `summarizeFleet` có test riêng cho ca **máy LỖI không được tính là "đã cập nhật đủ"** (`updates: []` + `error` → nếu chỉ xét độ dài mảng là nó thành "đã đủ", sai theo hướng làm người ta yên tâm). Bảng khoá i18n dùng `Record<UpdateGroup, I18nKey>` tường minh chứ **không ghép chuỗi khoá** — ghép thì đổi tên nhóm là mất dịch mà build vẫn xanh. **+ F36 quét xong không biết làm gì** (user báo: "phân tích xong rồi có đề xuất gì để người dùng dễ hình dung nên làm gì hơn không?"): danh sách `du` trả lời *"cái gì chiếm chỗ"* nhưng không trả lời *"giờ làm gì"*. Thêm `packages/shared/src/diskVerdict.ts` — `findFilesystem` + `diskVerdict`, khối kết luận đặt TRÊN danh sách: phân vùng nào / đầy bao nhiêu / còn bao nhiêu, thư mục này chiếm bao nhiêu % **phần đã dùng**, và **một câu nên làm gì kèm nút làm luôn**. 4 tình huống 4 câu khác nhau: `drillDown` (một mục con ≥60% → "vào `log`, phần còn lại gộp lại cũng không đáng kể" + nút), `filesHere` (file rời ≥40% → **đi sâu hơn không tìm ra gì** vì `du -d 1` chỉ đếm thư mục, ca `catalina.out` khổng lồ nằm thẳng trong thư mục), `spread` (không mục nào áp đảo → nói thẳng là rải rác, đừng ngụ ý có thủ phạm), `wrongBranch` (<10% phần đã dùng → đang đào nhầm nhánh + nút lên cấp). ⚠️ `findFilesystem` khớp **mount point dài nhất VÀ theo biên đường dẫn**: `startsWith` trần thì `/boot` nhận nhầm `/bootstrap`, và kết luận sẽ nói về **sai phân vùng** — có test riêng. ⚠️ Không có `df` thì **không bịa mức khẩn** (`level: 'ok'`, `shareOfUsedPercent: null`). `looseKb` kẹp ở 0 vì thư mục thiếu quyền bị `du` bỏ qua nên tổng con có thể vượt tổng cha. Đặt ở `shared` cùng lý do §5 như `pkgSummary`. **Cổng chất lượng: typecheck 3 package · 1344 test xanh (69 file) · build sạch · soát IP public 0 dòng.** ⚠️⚠️ **ĐÃ TỰ DÍNH §3 và đã sửa**: lúc viết test/comment/CHANGELOG cho phần cron tôi chép thẳng **tên user + host thật từ ảnh terminal user gửi** vào repo. Đã thay hết bằng bộ mẫu (`deploy`, `app-01`) và quét lại 0 dòng. **Bài học: dữ liệu trong ảnh chụp màn hình cũng là dữ liệu user cung cấp** — khi làm fixture cho một bug do user báo, phản xạ chép nguyên output của họ là rất mạnh, phải chặn ngay từ lúc gõ. Docs: CHANGELOG `[0.2.12]` (**đã CHUYỂN mục viết nhầm vào `[0.2.11]` sang đây** — v0.2.11 publish rồi mà không chứa nút × / tab tính năng, đúng cái bẫy §2), README (badge + test-count 1344 + bullet tab tính năng + bullet F37 tóm tắt + bullet F36 kết luận), USER-GUIDE, **landing `v0.2.8`→`v0.2.12`** (lệch 4 version). **CHƯA test GUI.** |
> | **v0.2.11** | ✅ **ĐÃ PHÁT HÀNH** — commit `1110ee4` + tag `v0.2.11` trỏ đúng HEAD (xác nhận 2026-08-31 bằng `git ls-tree`: tag **KHÔNG** có `FeaturesTabView`/`toolCatalog`, và `ui.tsx` lệch +16 dòng → nút × và tab tính năng KHÔNG nằm trong bản này). Mô tả lúc chuẩn bị release giữ bên dưới: |
> | ~~v0.2.11 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.11`, CHƯA commit/tag, CHƯA TEST GUI.** = **F30 xem log + F35 cron + F42 xoay vòng key**. User chọn 4 mục, **P46 SFTP queue CỐ Ý BỎ LẠI** (lý do ở cuối ô này). **(1) F30** (`connection/execStream.ts` + `ipc/logTail.ts` + `LogTailModal.tsx`): dựng **kênh exec dạng STREAM đầu tiên** của app ngoài phiên terminal và tunnel — `execOnce` là một-phát-rồi-đóng nên không dùng được. `LineBuffer` gom chunk thành dòng (chunk SSH cắt không theo ranh giới dòng; CRLF/CR quy về `\n`; `flush()` cho dòng cuối không có `\n`). **Không có timeout** (tail phải sống tới khi user tắt) nên kỷ luật dọn là phần quan trọng nhất: dọn ở **cả 3 đường** — user dừng, lệnh tự thoát, và **cửa sổ renderer đóng** (`sender.once('destroyed')` — đường này KHÔNG có sự kiện nào từ UI). Gộp dòng 100ms trước khi bắn IPC (log ồn ra hàng nghìn dòng/giây thì renderer chết chìm trong IPC chứ không phải trong việc vẽ). Dùng **`tail -F`** chứ không `-f`. Lọc/tô màu là hàm thuần ở `packages/shared/logView.ts` (renderer không import được core). ⚠️ **Test bắt lỗi thật**: regex gõ dở (`/[`) không "cho qua tất cả" như comment tôi viết — nó rơi xuống tìm chuỗi literal `"/[/"` và **lọc trắng panel**; phải tách `broken` khỏi `literal` và trả `true` TRƯỚC cả `invert` (đảo của "không lọc" vẫn là "hiện tất cả"). **(2) F35** (`diag/crontab.ts` + `CronModal.tsx`): parse giữ **NGUYÊN VĂN mọi dòng** kể cả dòng không nhận ra (dựng lại file từ những gì mình hiểu = xoá mất dòng của đồng nghiệp); `describeSchedule` trả **khoá i18n + params** để UI dịch, và trả `null` cho biểu thức phức tạp → hiện nguyên văn. Ghi: không `crontab -` (kênh exec không đẩy stdin), không heredoc, không `$?` → `printf` ra file tạm `umask 077` → `crontab <file>` → `rm -f` vô điều kiện; thành công nhận biết bằng **marker trong stdout** vì `rm` che exit code. Xác nhận trước khi ghi + cảnh báo đậm khi host thuộc nhóm PRODUCTION (F27). **(3) F42** (thêm vào `keys/authorizedKeys.ts` + `ipc/copyId.ts` + `KeyRotateModal.tsx`): **bất biến — key mới không đăng nhập được thì key cũ GIỮ NGUYÊN, trên mọi nhánh thất bại**. Chạy tuần tự, có nút dừng-sau-máy-hiện-tại. Gỡ key cũ bằng đọc → lọc trong JS → ghi lại cả file, **không `sed -i`**; `chmod 600` sau khi ghi (thiếu là sshd bỏ qua cả file = vừa gỡ cũ vừa hỏng mới). **Cổng chất lượng: typecheck 3 package · 1291 test xanh (68 file, chạy TỪ `packages/core` bằng Electron) · build sạch · soát IP public 0 dòng.** **CHƯA test GUI** — F30/F35/F42 đều phải thử trên máy Linux thật, và F42 thì phải thử trên máy KHÔNG quan trọng trước, giữ sẵn một phiên đang mở. ⛔ **P46 SFTP queue KHÔNG làm**: `SftpService` dùng `fastGet`/`fastPut` của ssh2 mà hai hàm đó **không có pause/resume**; làm thật phải viết lại đường truyền bằng `open`+`read`/`write` theo offset — đụng vào tính toàn vẹn file chứ không phải bọc thêm hàng đợi. Đã ghi phân tích vào ROADMAP, đề nghị tách 2 mốc (hàng đợi+retry+ETA rẻ; pause/resume mới là phần lớn). |
> | **v0.2.10** | ✅ **ĐÃ PHÁT HÀNH** — commit `c2caf2e` + tag `v0.2.10` trỏ đúng HEAD (xác nhận 2026-08-30). Mô tả lúc chuẩn bị release giữ bên dưới: |
> | ~~v0.2.10 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.10`, CHƯA commit/tag, CHƯA TEST GUI.** = **F36 đĩa đầy ở đâu + F37 máy nào cần vá + dọn nợ**. **(1) F36** (`diag/diskUsage.ts` + `DiskUsageModal.tsx`): `df` trước `du` vì câu hỏi đầu tiên là "phân vùng nào sắp đầy"; đi **một cấp mỗi bước** (`du -d 1`) chứ không quét cả cây; **`-x` bắt buộc** — thiếu nó `du /` bò vào `/proc`/`/sys`/mount mạng; `-k` ép KB khỏi phải parse "1.4G"; thư mục thiếu quyền bị bỏ qua chứ không hỏng cả lần quét (UI nói rõ nên tổng cấp cha > tổng các dòng). Tổng lấy từ DÒNG CHÍNH nó chứ không cộng con — cộng lại sẽ thiếu file nằm ngay trong thư mục, thường chính là thứ chiếm chỗ. 21 test. **(2) F37** (`diag/packageUpdates.ts` + `PackageUpdatesModal.tsx`): dò manager bằng `command -v` (không `which` — image tối giản không có), quét theo **lô 4** thay vì `Promise.all` cả loạt (30 kết nối cùng lúc qua một gate là cách chắc chắn để bị nghẽn), **chỉ đọc** — KHÔNG chạy `apt update` (cần root + ghi vào máy) và **không có nút vá**; có test khẳng định không lệnh nào chứa `install`/`upgrade`. 21 test. **(3) Nợ schema**: **vault v18** bỏ bảng chết `vpn_profiles` + cột `hosts.vpn_profile_id`. Đã **thử thật bằng node:sqlite** trước khi viết migration: SQLite 3.53 (bản đi kèm Electron) cho phép `DROP COLUMN` trên cột FK và giữ nguyên dữ liệu; thứ tự bắt buộc **bỏ CỘT trước, bỏ BẢNG sau** vì cột là đầu con của khoá ngoại. Cột này không nằm trong danh sách cột của `importSnapshot` nên sync không đụng. Thêm `migrations.test.ts` (3 test, gồm "host lưu trước khi migrate vẫn còn"). **(4) Dọn ROADMAP**: F32 và F04 **đã xong từ lâu mà vẫn ghi là chưa** — F32 có `MetricsStore`+`downsample`+`MetricsHistoryModal`, F04 có `AlertEngine`+webhook cấu hình ở `ipc/monitorSettings.ts` (JSON chứ không phải bảng `monitor_rules` như dòng cũ dự tính, cố ý: vault khoá sau 15 phút mà alert phải đọc ngưỡng liên tục). ⚠️ **ROADMAP đã lạc hậu 3 dòng trong 2 phiên liên tiếp** (F46, F32, F04) — cùng kiểu hỏng với dòng trạng thái release: mọi phiên lập kế hoạch sau bắt đầu từ bản đồ sai. **Cổng chất lượng: typecheck 3 package · 1235 test xanh (66 file, chạy TỪ `packages/core` bằng Electron) · build sạch · soát IP public 0 dòng.** **CHƯA test GUI** — riêng F36/F37 phải thử trên máy Linux thật, parser chỉ mới đối chiếu với output mẫu. |
> | **v0.2.9** | ✅ **ĐÃ PHÁT HÀNH** — commit `c839252` + tag `v0.2.9` trỏ đúng HEAD (xác nhận 2026-08-30). Mô tả lúc chuẩn bị release giữ bên dưới: |
> | ~~v0.2.9 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.9`, CHƯA commit/tag, CHƯA TEST GUI.** = **card nhóm host 3 vùng bấm + F27 + F43 + F44 + dải "Cần chú ý"**. **(1) Card nhóm** (`DashboardView.tsx` + `GroupHostsModal.tsx` mới): trước là MỘT `<button>` bọc cả card chỉ làm được một việc; nay tên nhóm (và chip `⊞ N`) mở danh sách host đầy đủ, **mỗi host một chip bấm được kèm chấm trạng thái của chính nó** (thay dãy chấm vô danh cũ — thấy "có máy chết" mà không biết máy nào), dòng chân giữ nguyên hành vi mở cả nhóm. ⚠️ Card KHÔNG còn là `<button>`: lồng button vào button là HTML không hợp lệ, trình duyệt tự gỡ lồng và mất nút con. Đã **tự chụp bằng Electron** để xem: ảnh đầu lộ dòng chân không neo đáy nên card trong cùng hàng lệch nhau → thêm `mt-auto`. **(2) F27** — guard lệnh nguy hiểm vốn chỉ khớp mẫu chuỗi, không biết host nào cũng không biết Broadcast đang bật (mà `sendData` fan-out ra mọi pane khi `tab.broadcast && panes.length > 1`); nay **vault v17** thêm cờ `production` ở GROUP (host kế thừa qua chuỗi group; chỉ cần MỘT nhóm bất kỳ trên đường lên gốc bật là tính, cố ý không cho nhóm con hạ mức), hộp thoại nói **số máy sắp nhận lệnh** + liệt kê máy production, và có production thì **bắt gõ lại tên máy**. ⚠️ Cột mới **phải khai thêm ở `importSnapshot`** nếu không sẽ mất khi sync — đúng loại regression đã có test cho `secret_ref`. **(3) F43 ssh-copy-id** (`ipc/copyId.ts` + `CopyIdModal.tsx`): ghi key rồi **đăng nhập thử bằng chính key đó** mới báo thành công (sai quyền `~/.ssh` thì sshd bỏ qua key mà KHÔNG báo gì — §8); so key theo `<type> <blob>` bỏ comment nên đẩy lại từ máy khác không ghi trùng; lệnh shell giữ mức tầm thường (`cat` + `printf >>`), phần "đã có chưa" so trong JS. **(4) F44** (`ipc/knownHosts.ts` + `KnownHostsModal.tsx`): xem/quên fingerprint đã TOFU, xoá ghi **tombstone** để lan sang máy khác qua sync. **(5) Dải "Cần chú ý"**: `collectAttention` gom host chết + tunnel lỗi + slave `critical`, CHỈ hiện khi có vấn đề. ⚠️ **Hai hàm thuần mới nằm ở `packages/shared` chứ không phải `packages/core`** (`commandScope.ts`, `attention.ts`) vì **renderer không import được `@infra/core`** (§5) mà guard chạy lúc bấm Enter nên cũng không đi vòng qua IPC được; **test vẫn để ở `packages/core`** vì vitest chỉ quét ở đó. Dọn thêm: ROADMAP dòng F46 sót lại chưa gạch (thực ra xong từ v0.1.9). Docs: CHANGELOG `[0.2.9]` + README (badge, 3 bullet mới, test-count **1190**) + USER-GUIDE (§1 production + ssh-copy-id, §15C fingerprint, mục Dashboard) + ROADMAP (F27/F43/F44/F46 + dải Cần chú ý). **Cổng chất lượng: typecheck 3 package · 1190 test xanh (63 file, chạy TỪ `packages/core` bằng Electron) · build sạch · soát IP public 0 dòng.** **CHƯA test GUI.** |
> | **v0.2.8** | ✅ **ĐÃ PHÁT HÀNH** — commit `0014272` + tag `v0.2.8` trỏ đúng HEAD (xác nhận 2026-08-29). Mô tả lúc chuẩn bị release giữ bên dưới: |
> | ~~v0.2.8 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.8`, CHƯA commit/tag, CHƯA TEST GUI.** = **F65 chống ghi đè sync + chuyển vault bằng file + auto-sync**. Xuất phát từ câu hỏi của user: "để blob lên Google Drive rồi sync trực tiếp trên Drive được không, quên đem máy thì tải file về máy khác chọn sync về?" — được, vì backend chỉ là **thư mục**; nhưng rà lại thì lộ 3 lỗ. **(1) Ghi đè im lặng — nghiêm trọng nhất.** `read()` trả `null` bị hiểu là "lần đầu, chưa có gì" nên app ghi blob của mình đè lên; mà đó là đúng hai tình huống hay xảy ra nhất: file **có ở đó nhưng sai tên** (`infra-companion-vault (1).blob` — tên trình duyệt đặt cho bản tải trùng) và **client cloud chưa tải xong**. Cả hai đều thay dữ liệu của mọi máy khác bằng dữ liệu máy này. Sửa: `findNearMissBlobs()` (hàm thuần, bắt tên chứa `infra-companion-vault` mà khác tên chuẩn) + `SyncBackend.listNearMisses()` + nhớ `seenRemoteAt` trong `SyncConfig`; `guardMissingRemote()` trúng một trong hai → trả `ok:false, needsConfirm:true, wrote:false` và **KHÔNG ghi**, nêu tên file tìm thấy. Vault trống + backend trống → `ok:true, wrote:false` (không tạo blob rỗng). Ghi đè là nút ⚠ riêng (`force`). **(2) Chuyển bằng file**: `SYNC_EXPORT_FILE`/`SYNC_IMPORT_FILE` (dialog cũ chỉ chọn THƯ MỤC nên kịch bản "máy mượn chỉ có trình duyệt" không làm được); tách `SyncService.buildBlob/applyBlob/saltOf` để dùng chung với đường thư mục. Nhập là **một chiều**, UI nói rõ. **(3) Auto-sync** 0/5/15/30/60 phút (mặc định 15) + **một lượt đẩy lúc thoát app** (`flushSyncOnQuit()` nối vào `before-quit`, nằm trong cùng `Promise.race` với `QUIT_GRACE_MS` để thư mục mạng treo không giữ app lại; chạy TRƯỚC `getVault().lock()` vì `exportSnapshot` cần DEK; auto-sync để "Tắt" thì cũng không đẩy). Timer `unref()`, cờ `running` chống chồng lượt, **bỏ lượt khi vault khoá**, **KHÔNG gọi `touchActivity()`** (sẽ vô hiệu auto-lock). Sự kiện `SYNC_PULLED_EVENT` → App.tsx `refreshAll()` + toast, vì auto-sync đổi dữ liệu dưới chân một cửa sổ đang hiện bản cũ thì tệ hơn không sync. ⚠️ **Bẫy đã dính và đã sửa trong lúc làm**: `applyAutoTimer()` chạy ngay trong `registerSyncIpc()`, mà `getSyncConfig()` gọi `ensureDb()` → **tạo `vault.db` trước cả màn hình đặt master password** trên máy mới, và mở DB lỗi thì ném ngay làm app không boot; fix = kiểm `state() === 'uninitialized'` (hàm này `existsSync` rồi trả sớm) + try/catch. `SyncConfig` thành interface export, thêm `autoMinutes?`/`seenRemoteAt?` — lưu JSON trong `meta` nên **không cần migration vault**. **+ P30 xuất hosts ra định dạng đọc được** (`⋯` → Xuất hosts…, `ExportHostsModal` + `ipc/export.ts` + `packages/core/src/exporters/hostExport.ts`, **25 test**): trước đó **không có exporter nào** — dữ liệu vào được mà ra thì không. `resolveForExport()` **phân giải kế thừa group trước** (host để trống `username`/`keyId`/`jumpChain` là lấy của group; xuất thô ra file ssh_config thiếu `User`, không đăng nhập được), `sshAlias()` bỏ `* ? ! #` (nhãn "web *" để nguyên thành pattern khớp MỌI host) + khử trùng lặp `-2`; `ProxyJump` dựng từ jump chain bằng alias thật, hop trỏ vào chính nó / host đã xoá bị loại. **Không chứa bí mật**: bỏ `notes`, `env`, `secretRef`, `hasPassword`, key material — có test riêng khẳng định cả 3 định dạng đều không rò. `IdentityFile` **để dạng comment** vì key nằm trong vault chứ không phải file trên đĩa (đoán path = config trông chạy được nhưng hỏng lúc connect). ⚠️ **Test bắt được lỗi thật lúc viết**: `sshAlias('web *')` ra `web-` vì thay khoảng trắng TRƯỚC khi bỏ ký tự nguy hiểm — đổi thứ tự + cắt gạch thừa hai đầu. Docs: CHANGELOG `[0.2.8]` + USER-GUIDE §13 viết lại + **§15B mới** + README (badge v0.2.8, 3 bullet Sync, test-count **1119**, Known Limitations v0.2.8 nói rõ chưa có UI xung đột và mức tối thiểu 8 ký tự là yếu) + ROADMAP (**F63 + F64 của v0.2.7 bị bỏ quên, nay bổ sung** + F65 + mục P30 export) + landing hero `v0.2.4`→`v0.2.8` (lệch 3 version). ⚠️ **Bẫy thứ hai — test đỏ vì TRANH CPU chứ không phải vì sai**: suite sync mới ban đầu gọi `deriveSyncKey` 6 lần; argon2id 19 MiB chạy pure-JS tốn cỡ giây mỗi lần, nên khi 58 file chạy song song nó làm `crypto.test.ts > sync key…` **timeout ở 5000ms** (mặc định của vitest) — đỏ dù logic đúng. Sửa **hai đầu**: dẫn xuất key MỘT lần ở module scope trong `sync.test.ts` (6 → 2 lần; cùng passphrase + salt luôn ra cùng key nên tương đương hoàn toàn) và cho riêng test nặng nhất kia timeout `30_000` kèm lý do. Sau đó **3 lần chạy full suite liên tiếp đều xanh**, tổng thời gian test giảm ~155s → ~105s. **Bài học: trước khi nghi logic, xem thông báo có phải "Test timed out" không — và đừng hạ tham số KDF để test nhanh lên.** **+ Xem lại bí mật đã lưu + F45 clipboard tự xoá** (`ipc/reveal.ts`, `RevealSecretModal.tsx`, nút 👁 ở form host và danh sách Keys): **ngoại lệ CÓ CHỦ ĐÍCH của quy tắc "mật khẩu không qua IPC sang renderer"** — đã sửa CLAUDE.md §4 cho khớp thay vì lách. Lập luận: quy tắc đó chống **lỗi phía renderer** quét credential cả fleet, không phải giấu dữ liệu khỏi user; mà blob sync vốn đã chứa `password_plain` của mọi host nên nó chưa bao giờ là ranh giới chống chính user. Cái giữ lại: **không bao giờ có bí mật trong DTO danh sách**. Guard: **luôn bắt nhập lại master password kể cả khi vault đang mở** (bật "ghi nhớ" → `touchActivity()` thoát sớm nên KHÔNG BAO GIỜ auto-lock + `tryAutoUnlock()` tự mở bằng DPAPI ⇒ vault gần như luôn mở; thiếu bước này là "mở khoá máy" = "mở khoá cả hạ tầng"), khoá tạm 60s sau 5 lần sai, che mặc định + tự ẩn 20s, giá trị chỉ ở state component (**không vào store**), nút **chép thẳng** đi đường riêng nên giá trị **không hề vào renderer**, clipboard tự xoá 30s + xoá cả khi thoát app (chỉ xoá nếu clipboard VẪN giữ đúng thứ mình ghi). Core thêm **`verifyMasterPassword()`** — KHÔNG dùng `unlock()` vì nó gán `this.dek` nên "kiểm tra" sẽ mở vault như tác dụng phụ (có test riêng khẳng định điều đó) — cùng `revealHostPassword()` / `revealKeyPassphrase()`. TOTP seed **cố ý không** xem lại được. ⚠️ **TS**: `'value' in result` KHÔNG thu hẹp được kiểu vì `RevealResult.value` là optional → phải dùng discriminated union bằng cờ `granted`.
⚠️⚠️ **BẪY LỚN NHẤT PHIÊN NÀY — lệnh test ghi trong CLAUDE.md §6 và README bị SAI, đã sửa**: chạy `electron … vitest.mjs run` **từ gốc repo** thì vitest không thấy `packages/core/vitest.config.ts` → mất `testTimeout: 30_000`, rơi về mặc định **5000ms**. Suite có argon2id 19 MiB pure-JS (~1s/lần) nên đỏ **ngẫu nhiên, mỗi lần một test khác** (đã thấy `crypto.test.ts`, `reveal.test.ts`, `RuntimeManager.test.ts`, `vaultMerge.test.ts`, `ManagedStackProvider.test.ts` — hầu hết chẳng liên quan crypto, chỉ đói CPU). Tôi đã **chẩn đoán sai một lần**: tưởng flake do suite mới nên thêm `testTimeout` 30s vào riêng `crypto.test.ts` — nay đã gỡ vì thừa. **Lệnh đúng: `Set-Location packages\core` rồi mới chạy.** Chạy đúng chỗ = **1153/1153 xanh**. Việc giảm số lần gọi argon2id trong `sync.test.ts`/`reveal.test.ts` thì vẫn giữ — đó là chuyện tốc độ (suite nhanh hơn ~50s), không phải chuyện đỏ/xanh. **Cổng chất lượng: typecheck 3 package · 1153 test xanh (60 file, chạy TỪ `packages/core` bằng Electron) · build sạch · grep bundle xác nhận `__RELEASE_NOTES__` = mục `[0.2.8]`, không lẫn `[0.2.7]` · soát IP public 0 dòng.** **CHƯA test GUI.** |
> | v0.2.7 | ✅ **ĐÃ commit `58b795a` + tag `v0.2.7` TRỎ ĐÚNG HEAD** — kiểm 2026-08-28 bằng `git rev-list -n1 v0.2.7` (= `58b795a` = HEAD). **Lần thứ 5 ghi chú nói "chưa commit/tag" trong khi đã publish** → dòng dưới là mô tả lúc chuẩn bị release, giữ làm hồ sơ. Trước khi tin bất kỳ dòng nào trong bảng này: `git tag --sort=-v:refname` + `git rev-list -n1 <tag>` + `git ls-tree -r --name-only <tag>` tìm file mới nhất. |
> | ~~v0.2.7 (mô tả cũ)~~ | 🟡 **SẴN SÀNG RELEASE — đã bump 2 `package.json` lên `0.2.7`, CHƯA commit/tag, CHƯA TEST GUI.** = **F63 ghim tunnel + đổi màu khu Yêu thích + F64 thêm SSH key ngay trong form host**. **F64**: dropdown SSH Key có option `+ Thêm key mới…` (sentinel `__newkey__`, cùng khuôn `NEW_GROUP`) mở panel inline sinh ed25519 / import key (OpenSSH·PEM·.ppk + passphrase), tạo xong **tự chọn** key đó cho host — store `generateKey`/`importKey` đổi chữ ký `boolean` → `SshKeyDto \| null` (IPC vốn đã trả DTO); Enter trong ô của panel = tạo key chứ KHÔNG submit form host; submit khi panel dở dang → lỗi `host.errKeyPending`; 12 khoá i18n mới `host.newKeyOpt`/`host.key*`/`host.errKeyPending` ×3 ngôn ngữ. **F63**: khối ★ Yêu thích ở sidebar tô nền/viền `warning` nhạt để khác hẳn group thường; tunnel ghim được bằng nút sao (`stores/favorites.ts` → factory `createFavoritesStore(key)`, thêm `useTunnelFavoritesStore` khoá localStorage `infra.tunnelFavorites`, helper `pinnedFirst()` partition ổn định giữ A→Z trong từng nửa, listener `storage` để cửa sổ tách rời sync với cửa sổ chính); tunnel ghim nổi lên đầu + vạch vàng trái ở **TunnelsModal**, ★ chỉ báo ở **Dashboard** và **cửa sổ tách rời** (bên đó không có nút toggle vì chật). Title nút sao dùng lại khoá `sidebar.favorite`/`sidebar.unfavorite`, không thêm khoá i18n mới. **Cổng chất lượng: typecheck 3 package · build sạch · grep bundle có `0.2.7` + ghi chú `[0.2.7]` và không còn `0.2.6` · soát IP public 0 dòng · test core KHÔNG chạy lại (chỉ chạm renderer)** |
