...
**Current release: v0.2.20 (Phase 0–6)**  &nbsp;|&nbsp; Windows · macOS · Linux  &nbsp;|&nbsp; Electron 42 · React 19 · TypeScript
...
### Terminal UX
- **Three themes — Infra default, Navigator and Workbench** — *Infra default* is the classic layout (host tree in the left column, Dashboard as home). *Navigator* is Termius-style: the left column is a **menu** (Hosts · SFTP · Tunnels · Snippets · SSH Keys · Workspaces · History · Tools, plus Dashboard if you want it — **you pick which show and drag the order**, Hosts is home) and nothing unfolds inside it — pick an entry and it opens in the **main area**: a **Hosts page** with favorites, groups as folder cards you step into, wide host rows with the usual actions and a search that flattens everything; a **History page**; and Tunnels / Snippets / Keys / Workspaces as pages, laid out in **two columns** when the window is wide. *Workbench* is VS Code-style: a 48px activity bar picks a **resizable side panel** (hosts, tunnels, snippets, keys, workspaces, history, tools) while the main area stays on your terminals, plus a **`Ctrl+J` bottom panel** under the terminal for Monitoring, Log and Tunnels. Dark/Light is a separate *Color mode* (Settings → Appearance → Theme)
...
- **Scheduled jobs** — run a command, a snippet or an inventory collection on a **cron schedule inside the app** (no server crontab): presets and a live *next run* preview, per-host results with exit code, duration and captured output, the last 50 runs per job, failures recorded in Notifications. The schedule lives outside the vault so it is known the moment the app starts; a run that meets a **locked vault is recorded as skipped with the reason**, never silently dropped
- **Fleet security check** — one read-only command per host, scored out of 100: sshd still accepting passwords or root, ports listening **beyond loopback**, failed logins, missing fail2ban, `NOPASSWD` sudo, empty passwords, a second UID 0, root's `authorized_keys`, firewall, SELinux, pending reboot — each finding links to the matching runbook. Nothing is changed, no `sudo`, and results are **never written to disk**
- **Folder diff & auto-upload** — pair a local folder with a folder on a host: a diff table by modified time and size (same time, different size shows as a conflict rather than a guess), **Push changes** for the local-newer and local-only files, and **Watch** to upload on save, reusing one SFTP session. One way only, machine → host; a file newer on the server is flagged, not overwritten, and a local delete never deletes remotely

### Terminal that knows where each command starts and ends
- **Shell integration (OSC 133)** — add a small snippet to the host's `.bashrc` (Settings copies it) and the pane shows the last command's **exit status and how long it ran**
- **AI assistant in a docked column** — `Ctrl+I` opens it on the right; the column takes real space so the terminal narrows rather than being covered, because half of asking about a server is reading its output. Drag the left edge to resize, and the question box stays pinned at the bottom so a long answer never pushes it out of reach. A suggested command is **inserted, not run**, and the row above the button names the machine it will go into. The assistant and the AI troubleshooter **share one column with tabs** rather than opening a second one — two columns would leave the terminal a sliver, which is the thing they both exist to let you watch
- **Command history per host** — `Ctrl+Shift+R` searches every command you have run, on any machine, months back — even when the server wiped its own `.bash_history`. Picking one **inserts it into the terminal without running it**, so you read it first and the dangerous-command guard still applies; `Ctrl+R` keeps reaching the remote shell. Passwords typed on a command line are redacted before storing, the text is encrypted with the vault key, and the history stays on this machine (export/import by file, not sync)
- **Long-command notification** — a command over 20 seconds tells you when it finishes, but only if you were looking elsewhere (another tab, or the window unfocused); interactive programs (editors, `top`, pagers) never notify

### Codex agent — on your ChatGPT plan, no API key
- **OpenAI's agent, embedded** — the app runs `codex app-server` as a child process and talks JSON-RPC to it, the same interface the Codex CLI, the Codex desktop app and the VS Code extension use. Pick a working folder and the agent reads and edits files and runs commands there, streaming its answers, the commands and their output into the panel as they happen
- **Signed in with your plan** — authentication is whatever `codex login` already stored; the app **never reads** `~/.codex/auth.json` and never passes `OPENAI_API_KEY` to the agent, so a key in your environment cannot quietly start billing per token. Sign in, switch account and sign out are buttons in the app (browser flow, or a device code where no browser opens), and the panel shows the account, the plan and how much quota is left
- **Several accounts** — profiles, because Codex keeps one account per config directory. The default profile shares `~/.codex` with the `codex` in your terminal; a separate one has its own sign-in
- **Earlier sessions resume** — Codex already writes each session's transcript under `~/.codex/sessions`, so the panel lists them and reopening one continues the conversation with everything the agent knows. No second copy is kept: two sources of truth for one thing drift apart
- **Model and reasoning depth in the chat box** — a throwaway question can run `low`, a refactor `high`; the list comes from the server, since which models exist changes with the CLI version and your plan
- **The app updates Codex** — `codex update` refuses to run when the CLI came from the Codex desktop app, and a stale CLI eventually makes *every* model fail. One button fetches the current release into the app's own folder, verifies it runs, and prefers it over PATH — without touching the `codex` you use in a terminal

### 3D assistant — a companion that notices when something breaks
- **Your own `.vrm` model, or one click to get one** — load any VRM file from your machine; the app **never downloads a model on its own and keeps no copy**, it remembers the path and shows the author and licence the file itself declares. No model? A CC0 character by pixiv downloads from inside the app, checksum-verified. It is **not bundled into the installer** — 14 MB would ship to everyone including the majority who never turn it on
- **It reacts to your fleet** — a monitoring threshold, replication falling behind or a failed URL check changes its expression, holds a worried posture, and says what happened in a speech bubble. Minimise to the tray and it still reaches you: the assistant appears on your desktop in a transparent frameless window that **does not steal focus** from whatever you are typing in, and clicking it brings the app back
- **Ask the AI without leaving it** — press and hold to open a chat bubble backed by the same assistant the rest of the app uses. *"open tunnels"* opens that tool, matched by keyword rather than sent to the model, so it works before AI is configured and cannot be talked into opening the wrong thing
- **Right-click for a radial menu** — chat at the top of the circle and settings at the bottom, the two ends of the vertical axis; expressions, outfits and motion down the right, model switching and your pinned Dashboard tools on the left. Its centre is a button: **✕ Đóng** to close, or **↩ Menu** to step back out of a sub-ring. Outfits are combinations of visible meshes **you** save and name, because a VRM file carries no notion of an outfit: measured on real models there is only a pile of separate meshes, machine-generated names, or everything merged into one
- **Arms that move like arms** — resting pose and elbows are solved with inverse kinematics against a target beside the hip, so the chain adapts to each model's proportions instead of being tuned for one skeleton. Elbows drift rather than locking, arms lead and trail as the body turns, and tugging the assistant moves hips and knees too instead of bending it at the waist
...
- Or **sign in with Google** and sync straight through the Drive API — no desktop sync client needed. Uses the `drive.file` scope, so the app can only touch the one file it created; a new machine needs just the Google sign-in plus the sync passphrase. **Signing in never replaces the passphrase**: the blob is encrypted before upload, so a stolen Google account yields one unreadable file, not your infrastructure
- **Channels run side by side** — folder, Google Drive, **WebDAV** (Nextcloud/Seafile/Synology/Nginx dav) and **S3** (AWS/MinIO/R2/B2, SigV4 with no SDK) in any mix, each with its own passphrase and status, each switchable off without touching the other. Server credentials are stored encrypted and only after a successful connection
...
- **Cloud import — DigitalOcean, AWS EC2, Google Cloud, Azure** — add a **read-only** account (DO token · IAM key with just `ec2:DescribeInstances` · GCP service-account JSON with *Compute Viewer* · Azure app registration with *Reader*), fetch, tick, import: hosts get their public IP first, private as fallback, and their origin recorded in the notes. Machines whose address already has a host are locked out, so re-importing never duplicates the list. Accounts are saved only after a successful fetch, credentials live **encrypted in the vault** and never cross into the UI process; the app only ever **reads** from the APIs
...
```bash
pnpm test
# 1415 tests; on Node 20 the node:sqlite suites (vault-merge, replication clusters, local-dev
...
- Sync backends: **folder, Google Drive, WebDAV and S3** (Git planned — see [ROADMAP.md](ROADMAP.md)); moving the blob by hand is covered by export/import to a file. There is no conflict-resolution UI — the merge is Last-Write-Wins and it does not ask. The folder backend's passphrase minimum is 8 characters (remote backends require 12, since that blob lives off-machine holding your keys and host passwords behind that one passphrase). Cloud import reads only the first page on accounts with thousands of machines (a warning says so)
...
- Plugin system (F16)
- Sync backends: Git (WebDAV and S3 shipped in v0.2.16)
- Cloud host import — remaining providers (Hetzner; AWS EC2 / GCP / Azure shipped in v0.2.16, DigitalOcean in v0.2.13)
